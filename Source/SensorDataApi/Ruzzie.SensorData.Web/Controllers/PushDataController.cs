...
namespace Ruzzie.SensorData.Web.Controllers
{
    public class PushDataController : ApiController
    {
        ...
        /// <returns></returns>
        public async Task<PushDataResult> Post(string thing, [FromBody] JToken content)
        {
            return await PushDataService.PushDataAsync(thing, DateTime.Now, content == null ? string.Empty : content.ToString());
        }
    }
}